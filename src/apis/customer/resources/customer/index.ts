...
export const getDesignerWorkspace = async (userId: number): Promise<GetDesignerWorkspaceResponse> => {
  const res = await CustomerAPI.get<GetDesignerWorkspaceResponse>(`/v1/${userId}/shop`);
  return res.data;
};
...
export const getDesignerBadgesForCustomer = async (designerId: number): Promise<GetDesignerBadgesForCustomerResponse> => {
  const res = await CustomerAPI.get<GetDesignerBadgesForCustomerResponse>(`/v1/designers/${designerId}/badges`);
  return res.data;
};
