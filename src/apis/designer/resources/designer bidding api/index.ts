...
export const getEstimateAndProposalDetails = async (userId: number, processId: number, threadId: number): Promise<GetEstimateAndProposalDetailsResponse> => {
  const res = await DesignerAPI.get<GetEstimateAndProposalDetailsResponse>(`/v1/users/${userId}/bidding/processes/${processId}/threads/${threadId}`);
  return res.data;
};

export const getStep2Threads = async (userId: number): Promise<GetThreadsByStepResponse> => {
  const res = await DesignerAPI.get<GetThreadsByStepResponse>(`/v1/users/${userId}/bidding/processes/threads/step2`);
  return res.data;
};

export const getStep1Threads = async (userId: number): Promise<GetThreadsByStepResponse> => {
  const res = await DesignerAPI.get<GetThreadsByStepResponse>(`/v1/users/${userId}/bidding/processes/threads/step1`);
  return res.data;
};

export const getStep3AboveThreads = async (userId: number): Promise<GetThreadsByStepResponse> => {
  const res = await DesignerAPI.get<GetThreadsByStepResponse>(`/v1/users/${userId}/bidding/processes/threads/above-step3`);
  return res.data;
};
